...
class Animal {
  constructor({ animal_id, user_id, animal_type, count, wellbeing, influence, info }) {
  ...
  //CREATE ONE
  static async createAnAnimal(user_id, { animal_type, influence, info }) {
    const values = [user_id, animal_type, influence, info];

    const response = await db.query(
      "INSERT INTO animal (user_id, animal_type, influence, info) VALUES ($1, $2, $3, $4) RETURNING *;",
      values
    );
    return new Animal(response.rows[0]);
  }
